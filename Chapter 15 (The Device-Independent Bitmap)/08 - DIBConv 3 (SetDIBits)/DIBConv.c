...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindow(
                szAppName,
                TEXT("DIB to DDB Conversion using SetDIBits()"),
                WS_OVERLAPPEDWINDOW,
                CW_USEDEFAULT,
                CW_USEDEFAULT,
                CW_USEDEFAULT,
...
HBITMAP CreateBitmapObjectFromDibFile(HDC hdc, PTSTR szFileName)
{
    ...
    if(hBitmap)
    {
        if( ((BITMAPCOREHEADER *)(pbmfh + 1))->bcSize == sizeof(BITMAPCOREHEADER))
            cyDib = ((BITMAPCOREHEADER *)(pbmfh + 1))->bcHeight;
        else
            cyDib = ((BITMAPINFOHEADER *)(pbmfh + 1))->biHeight;
