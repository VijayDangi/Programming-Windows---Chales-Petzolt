/*------------------------------------------------
    ShowDIB2.c :- Show a DIB in the Client area
------------------------------------------------*/
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    int ShowDib(HDC, BITMAPINFO *, BYTE *, int, int, int, int, WORD);
        ...
        case WM_COMMAND:
            ...
            switch(LOWORD(wParam))
            {
                case IDM_FILE_OPEN:
                    ...
                    if(pbmfh == NULL)
                    {
                        MessageBox(hwnd, TEXT("Cannot load DIB file"), szAppName, MB_ICONEXCLAMATION | MB_OK);
                ...
                case IDM_FILE_SAVE:
                    ...
                    if(!bSuccess)
                        MessageBox(hwnd, TEXT("Cannot Save DIB file"), szAppName, MB_ICONEXCLAMATION | MB_OK);
                ...
                case IDM_FILE_PRINT:
                    ...
                    if(!bSuccess)
                        MessageBox(hwnd, TEXT("Could not print bitmap"), szAppName, MB_ICONEXCLAMATION | MB_OK);
                ...
                case IDM_EDIT_CUT:
                    ...
                    CopyMemory(
                        pGlobal,    //to
                       (BYTE *)pbmfh  + sizeof(BITMAPFILEHEADER),   //from
                        pbmfh->bfSize - sizeof(BITMAPFILEHEADER)    //how much
                    );
                    
                    GlobalUnlock(hGlobal);
                    
                    //Transfer it ot the clipboard
                    OpenClipboard(hwnd);
