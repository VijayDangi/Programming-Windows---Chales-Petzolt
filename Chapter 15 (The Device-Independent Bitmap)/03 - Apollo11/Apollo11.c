/*--------------------------------------------
    Apollo11 :- Program for screen captures
--------------------------------------------*/

//Headers
#include <windows.h>
#include "DibFile.h"    //for DibLoadImage()
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        case WM_CREATE:
            pbmfh[0] = DibLoadImage(TEXT("Apollo11.bmp"));  //bottom-up version
            pbmfh[1] = DibLoadImage(TEXT("ApolloTD.bmp"));  //top-down version
            ...
            pbmi[0] = (BITMAPINFO *) (pbmfh[0] + 1);  //=> pbmfh[0] + 1 * sizeof(BITMAPFILEHEADER)
            pbmi[1] = (BITMAPINFO *) (pbmfh[1] + 1);  //=> pbmfh[1] + 1 * sizeof(BITMAPFILEHEADER)
            
            pBits[0] = (BYTE *) pbmfh[0] + pbmfh[0]->bfOffBits;
            pBits[1] = (BYTE *) pbmfh[1] + pbmfh[1]->bfOffBits;
            
            //Get the DIB width and height (assumes BITMAPINFOHEADER)
            //Note that cyDib is the absolute value if the header value!!!
