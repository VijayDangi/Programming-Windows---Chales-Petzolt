    ...
    switch(message)
    {
        ...
        case WM_SETTINGCHANGE:
            ...
            while(GetMenuItemCount(hMenu) > 1)
                ...
                //
                //First, find out how large an array we need; this call 
                // will fail, leaving the required size in dwNeeded
                //
        ...
        case WM_COMMAND:
            ...
            if(LOWORD(wParam) == IDM_SCREEN || LOWORD(wParam) < IDM_DEVMODE)    //IDM_SCREEN & Printers
            {
...
void DoBasicInfo(HDC hdc, HDC hdcInfo, int cxChar, int cyChar)
{
    ...
    {
        HORZSIZE,           TEXT("HORZSIZE         Width in millimeter:"),
        VERTSIZE,           TEXT("VERTSIZE         Height in millimeter:"),
        ...
        ASPECTX,            TEXT("ASPECTX          Relative width in pixel:"),
        ASPECTY,            TEXT("ASPECTY          Relative height in pixel:"),
        ...
        PHYSICALWIDTH,      TEXT("PHYSICALWIDTH    Printerpage pixel width:"),
        PHYSICALHEIGHT,     TEXT("PHYSICALHEIGHT   Printer page pizel height:"),
    ...
    for(i = 0; i < sizeof(info)/sizeof(info[0]); i++)
    {
        TextOut(
            ...
            (i+1) * cyChar,
            szBuffer,
            wsprintf(szBuffer, TEXT("%-45s%8d"), info[i].szDesc, GetDeviceCaps(hdcInfo, info[i].nIndex))
        );      
    }
}
...
void DoOtherInfo(HDC hdc, HDC hdcInfo, int cxChar, int cyChar)
{
    ...
    static TCHAR *szTech[] = {
        ...
        TEXT("DT_RASPRINTER (Rstaer printer)"),
    ...
    TextOut(
        ...
        cyChar,
        szBuffer, 
        wsprintf(szBuffer, TEXT("%-24s%04XH"), TEXT("DRIVERVERSION:"), GetDeviceCaps(hdcInfo, DRIVERVERSION))
    );
    ...
    TextOut(
        ...
        2 * cyChar,
        szBuffer, 
        wsprintf(szBuffer, TEXT("%-24s%-40s"), TEXT("TECHNOLOGY:"), szTech[GetDeviceCaps(hdcInfo, TECHNOLOGY)])
    );
...
void DoBitCodedCaps(HDC hdc, HDC hdcInfo, int cxChar, int cyChar, int iType)
{
    ...
    static BITS text[] =
    {
        ...
        TC_SF_X_YINDEP,     TEXT("TC_SF_X_YINDEP  Can do independent of X and Y:"),
        TC_SA_DOUBLE,       TEXT("TC_SA_DOUBLE    Can do double character for scaling:"),
