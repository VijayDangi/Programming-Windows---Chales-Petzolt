...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		...
		case WM_PAINT:
		{
			...
			SelectObject(hdc, CreateHatchBrush(HS_HORIZONTAL, RGB(125, 200, 87)));			
			...
			TextOut(hdc, apt[0].x, 2 * (tm.tmHeight + tm.tmExternalLeading), str, wsprintf(str, TEXT("HATCH_BRUSH style : HS_HORIZONTAL")));
			...
			DeleteObject(SelectObject(hdc, CreateHatchBrush(HS_VERTICAL, RGB(125, 200, 87))));
			...
			TextOut(hdc, apt[0].x, 2 * (tm.tmHeight + tm.tmExternalLeading), str, wsprintf(str, TEXT("HATCH_BRUSH style : HS_VERTICAL")));
