...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch(message)
    {
        case WM_CREATE:
            TimerId = SetTimer(
                    NULL,
                    0, //if 1st parameter set to NULL then this parameter is ignored. Instead, the function returns a timer ID
                    1000,
                    TimerProc
                 );
            return(0);
    ...
    )
{
    ...
    /* KillTimer call purges the message queue of any pending WM_TIMER messages */
    KillTimer(
        hWnd,  //In this case hWnd is NULL as in SetTimer() is NULL
        TimerId //this is value returned from SetTimer()
       );
    ...
    SetTimer(hWnd, TimerId, 1000, TimerProc);
    // printf("%d %d\n", dwTime, GetTickCount());
}